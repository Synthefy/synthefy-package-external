...
def embed_ts(X: np.ndarray, m: int, padding) -> np.ndarray:
    """
    ...
    """
    X = X.copy()[::-1]
    assert isinstance(padding, str), "padding must be a string"
    assert padding in ["constant", "edge", "reflect", "symmetric"]
    if padding:
        if len(X.shape) == 1:
            X = np.pad(X, ((m, m)), mode=padding)  # type: ignore
        if len(X.shape) == 2:
            X = np.pad(X, [(m, m), (0, 0)], mode=padding)  # type: ignore
        if len(X.shape) == 3:
            X = np.pad(X, [(0, 0), (m, m), (0, 0)], mode=padding)  # type: ignore
...
class ConvergentCrossMapping:
    ...
    def compute_crossmap(self, Xe, Y, X=None, stride=-1, tpred=0, tol=1e-10):
        ...
        for i in range(m):
            ...
            wgts = np.exp(
                -dists / dmin
            )  # (k, nt) weights of k neighbors for each point

            y_pred = np.sum(Y[:, idx.T] * wgts.T[None, ..., None], axis=2)
            y_pred, y_target = (
                y_pred[:, :, 0].copy(),
                Y[:, : y_pred.shape[1], 0].copy(),
            )
    ...
    def fit(self, X, y=None):
        ...
        if self.library_sizes is None:
            if self.max_library_size is None:
                ...
                self.library_sizes = np.unique(
                    (
                        ...
                        ** np.arange(
                            ...
                            int(
                                np.floor(
                                    np.log(self.n / (self.d_embed + 1))
                                    / np.log(self.dilation_factor)
                                )
                            ),
                        )
                    ).astype(int)
                ...
                self.library_sizes = np.unique(
                    np.linspace(
                        1,
                        int(np.floor(self.n / (self.d_embed + 1))),
        ...
        if not np.all(np.diff(self.library_sizes) <= 0):
            warnings.warn(
                "Stride sizes must decrease monotonically. Sorting library sizes."
            )
            self.library_sizes = np.sort(self.library_sizes)[::-1]

        all_causmat = np.zeros(
            (len(self.library_sizes), X.shape[1], X.shape[1])
        )

        ## Iterate over library sizes to test robustness of causal matrix
        Xe = embed_ts(X, m=self.d_embed, padding="constant")
        ...
        for i, stride in enumerate(self.library_sizes):
            ...
            if (
                self.minibatch
                and Xe[:, ::stride].shape[1] > self.minibatch_size
            ):
                subset_inds = subset_inds[: self.minibatch_size]

            all_causmat[i] = self.compute_crossmap(
                Xe[:, subset_inds], y[: -(self.d_embed - 1)][subset_inds]
            )
        ...
        assert isinstance(rho_mono, np.ndarray), (
            "rho_mono must be a numpy array"
        )
