...
class MetaDataEncoder(torch.nn.Module):
    def __init__(self, dataset_config, denoiser_config, device):
        ...
        self.denoiser_config: DenoiserConfig = denoiser_config
        ...
        if self.combined_condition_encoder_exists:
            # TODO: @somi explain why we are doing self.denoiser_config.metadata_encoder_config.channels * 2
            # here.
    ...
    def forward(self, discrete_conditions, continuous_conditions):
        ...
        # If combined and discrete conditions are not present, then pos_emb is the only input.
        if (
            self.discrete_condition_encoder_exists
            or self.continuous_condition_encoder_exists
        ):
            combined_conditions = combined_conditions + pos_emb
        else:
            combined_conditions = pos_emb
...
class TimestampEncoder(torch.nn.Module):
    def __init__(self, configuration: Configuration):
        ...
        self.dataset_config = configuration.dataset_config
        self.denoiser_config = configuration.denoiser_config
        ...
        num_timestamp_labels = self.dataset_config.num_timestamp_labels

        self.timestamp_encoder = FCEncoder(
            embedding_dim=num_timestamp_labels,
            projection_dim=self.denoiser_config.metadata_encoder_config.channels,
            dropout=0.1,
        )
    ...
    def forward(self, timestamps):
        ...
        timestamps = timestamps + pos_emb
        metadata_enc = self.condition_transformer_encoder(
            timestamps.permute(1, 0, 2)
        )  # L, B, C, for the transformer to act across the time dimension
        metadata_enc = metadata_enc.permute(1, 0, 2)
        return metadata_enc  # B, L, C
...
class DiffusionTransformer(nn.Module):
    def __init__(self, config):
        ...
        if self.denoiser_config.use_periodic_projection:
            self.input_projection = Conv1d_with_init(
                1, self.channels, 1
            )  # just scales each element self.channels times
        else:
    ...
    def forward(self, denoiser_input):
        ...
        if self.denoiser_config.use_periodic_projection:
            x = noisy_input.reshape(B, K * L).unsqueeze(
                1
            )  # (B,1,K*L), scales the input by different self.channels
