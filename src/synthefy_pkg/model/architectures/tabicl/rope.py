...
def apply_rotary_emb(
    ...
    t : Tensor
        Input tensor to apply embeddings to
    start_index : int, default=0
        Starting index for applying rotation
    scale : float, default=1.0
        Scaling factor for embeddings
    seq_dim : int, default=-2
        Dimension containing sequence length
    """
    seq_dim = -2 if seq_dim is None else seq_dim
    scale = 1.0 if scale is None else scale  # Handle None case for scale
...
class RotaryEmbedding(nn.Module):
    ...
    def rotate_queries_or_keys(self, t, seq_dim=None, offset=0, scale=None):
        ...
        seq = self.get_seq_pos(
            seq_len, device=device, dtype=dtype, offset=offset
        )
    ...
    def get_scale(
        ...
        actual_seq_len = seq_len if seq_len is not None else t.shape[0]

        if (
            should_cache
            and exists(self.cached_scales)
            and (actual_seq_len + offset) <= self.cached_scales.shape[0]
        ):
            return self.cached_scales[offset : (offset + actual_seq_len)]
        ...
        # Convert scale to tensor if it's a float
        if isinstance(scale, float) or isinstance(scale, int):
            scale = torch.tensor(scale, device=t.device)
    ...
    def get_axial_freqs(self, *dims):
        ...
        for ind, dim in enumerate(dims):
            ...
            all_axis: list[Any] = [None] * len(dims)
            all_axis[ind] = Colon
    ...
    @torch.autocast("cuda", enabled=False)
    def forward(
        self, t: Tensor, seq_len: Optional[int] = None, offset: int = 0
    ) -> Tensor:
        should_cache = (
