...
class TabICL(nn.Module):
    ...
    def __init__(
        ...
        skip_col_embedding: bool = False,  # TODO: not implemented yet
        full_reg_decoder_config: Optional[TokenDecoderConfig] = None,
        embedder_name: str = "",
        embedder_config: str = "",
        embedder_checkpoint: str = "",
        train_as_univariate_forecast: bool = False,  # TODO: not implemented yet
    ...
    def _train_forward(
        ...
        X, univariate_pred_forecast = self.embedder(
            X, tabicl_target_mask=target_mask
        )
        ...
        if target_mask is not None:
            out = self.icl_predictor(
                representations, y_train=y_train, target_mask=target_mask
            )
    ...
    def _inference_forward(
        ...
        X, univariate_pred_forecast = self.embedder(X)
        ...
        if target_mask is not None and self.train_as_univariate_forecast:
            return {
                "univariate_forecast": univariate_pred_forecast[
                    target_mask.transpose(2, 1) == 1
                ],
