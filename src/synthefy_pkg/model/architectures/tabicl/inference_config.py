...
class MgrConfig:
    ...
    def __init__(
        ...
        min_batch_size: int,
        safety_factor: float,
        offload: bool | Literal["auto"],
        auto_offload_pct: float,
...
class InferenceConfig:
    ...
    COL_CONFIG: Optional["MgrConfig"] = None
    ROW_CONFIG: Optional["MgrConfig"] = None
    ICL_CONFIG: Optional["MgrConfig"] = None

    def __post_init__(self):
        if isinstance(self.COL_CONFIG, dict):
            self.COL_CONFIG = MgrConfig(**self.COL_CONFIG)  # type: ignore
        ...
        if isinstance(self.ROW_CONFIG, dict):
            self.ROW_CONFIG = MgrConfig(**self.ROW_CONFIG)  # type: ignore
        ...
        if isinstance(self.ICL_CONFIG, dict):
            self.ICL_CONFIG = MgrConfig(**self.ICL_CONFIG)  # type: ignore
