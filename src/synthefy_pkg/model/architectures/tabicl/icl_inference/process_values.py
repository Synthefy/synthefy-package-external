...
class TransformToNumerical(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        # Handle categorical columns (including pandas category dtype)
        cat_cols = []
        for col in X.columns:
            dtype = X[col].dtype
            if (
                dtype == np.dtype("object")
                or dtype == np.dtype("bool")
        ...
        numeric_cols = make_column_selector(dtype_include=np.dtype("float64"))(
            X
        )
...
class CustomStandardScaler(BaseEstimator, TransformerMixin):
    ...
    def inverse_transform(self, X):
        """Inverse transform features by reversing the scaling.

        Supports both single feature (target) and multiple feature inversions.
        """
        check_is_fitted(self)
        X = check_array(X)
        # Normalization
        if np.isnan(X).any():
            # Use column-wise mean imputation for missing values
            X_means = np.nanmean(X, axis=0)
            for col_idx in range(X.shape[1]):
                if np.isnan(X[:, col_idx]).any():
                    col_mean = X_means[col_idx]
                    X[:, col_idx] = np.where(
                        np.isnan(X[:, col_idx]), col_mean, X[:, col_idx]
                    )
...
class RTDLQuantileTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        normalizer = QuantileTransformer(
            output_distribution=self.output_distribution,  # type: ignore
...
class PreprocessingPipeline(BaseEstimator, TransformerMixin):
    ...
    def inverse_transform(self, X):
        ...
        if self.normalizer_ is not None:
            if self.normalization_method == "power":
                X = self.normalizer_.inverse_transform(X)
            elif self.normalization_method == "quantile":
                X = self.normalizer_.inverse_transform(X)
            elif self.normalization_method == "quantile_rtdl":
                X = self.normalizer_.inverse_transform(X)
            elif self.normalization_method == "robust":
                # RobustScaler doesn't have inverse_transform, so we skip it
                pass
        # Standard scaling
        X = self.standard_scaler_.inverse_transform(X)
        return X
    ...
    def shuffle(self, n_estimators: int) -> List[np.ndarray]:
        ...
        if self.method == "shift":
            ...
            if self.n_features <= 5:
                all_perms = [
                    list(perm)
                    for perm in itertools.permutations(feature_indices)
                ]
            ...
            raise ValueError(
                f"Unknown method: {self.method}. Use 'shift', 'random', 'latin', or 'none'."
            )

        shuffle_patterns = [np.array(pattern) for pattern in shuffle_patterns]
...
class EnsembleGenerator(BaseEstimator, TransformerMixin):
    ...
    def _generate_ensemble(self):
        ...
        shuffler = FeatureShuffler(
            self.n_features_in_, self.feat_shuffle_method, self.random_state
        )
        shuffle_patterns = shuffler.shuffle(self.n_estimators)

        if self.class_shift and self.n_estimators > 1:
            shift_offsets = self.rng_.sample(
                range(self.n_classes_), self.n_classes_
            )
        else:
            shift_offsets = [0]

        shuffle_shift_configs = list(
            itertools.product(shuffle_patterns, shift_offsets)
        )
        ...
        shuffle_shift_norm_configs = shuffle_shift_norm_configs[
            : self.n_estimators
        ]

        # Reorganize configs so that those with the same normalization method are grouped together
        used_methods = list(
            set([config[1] for config in shuffle_shift_norm_configs])
        )
        ...
        for method in used_methods:
            ...
            shuffle_patterns[method] = [
                config[0] for config in shuffle_shift_configs
            ]
            shift_offsets[method] = [
                config[1] for config in shuffle_shift_configs
            ]
