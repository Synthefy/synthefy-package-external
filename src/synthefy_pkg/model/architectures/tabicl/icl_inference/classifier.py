...
class TabICLClassifier(ClassifierMixin, BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if OLD_SKLEARN:
            ...
            y = check_array(y, dtype=None, force_all_finite=False)  # type: ignore
        else:
            X = check_array(X, dtype=None, force_all_finite=False)  # type: ignore
            y = check_array(y, dtype=None, force_all_finite=False)  # type: ignore
        ...
        assert self.y_encoder_.classes_ is not None
        self.n_classes_ = len(self.y_encoder_.classes_)

        if (
            self.n_classes_ > self.model_.max_classes
            and not self.use_hierarchical
        ):
            raise ValueError(
    ...
    def predict_proba(self, X):
        ...
        for norm_method, (Xs, ys) in data.items():
            shuffle_patterns = (
                self.ensemble_generator_.feature_shuffle_patterns_[norm_method]
            )
        ...
        if self.average_logits:
            avg = self.softmax(
                avg, axis=-1, temperature=self.softmax_temperature
            )
...
def main():
    """Main function for testing TabICLClassifier with example data.

    This function loads example data from a specified folder path,
    combines numerical and categorical features, and demonstrates
    the basic usage of TabICLClassifier.
    """
    import argparse
    import os
    import sys
    ...
    parser = argparse.ArgumentParser(
        description="Test TabICLClassifier with example data."
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="IBM_HR_Analytics_Employee_Attrition_and_Performance",
        help="Name of the dataset to use",
    )
    args = parser.parse_args()

    # Construct folder path using the dataset name from args
    base_path = "/workspace/data/talent_benchmark/data"
    folder_path = os.path.join(base_path, args.dataset)

    # Load data
    X_train, y_train = load_data(folder_path, split="train")
    X_test, y_test = load_data(folder_path, split="test")
