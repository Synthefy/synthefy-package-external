...
from synthefy_pkg.model.architectures.tabicl.attention import (
    multi_head_attention_forward,
)
from synthefy_pkg.model.architectures.tabicl.rope import RotaryEmbedding
...
class ClassNode:
    ...
    def __init__(self, depth: int = 0):
        ...
        self.child_nodes = []
        self.class_mapping = {}
        self.classes_: Optional[Tensor] = None
        self.group_indices: Optional[Tensor] = None
...
class SkippableLinear(nn.Linear):
    ...
    def __init__(
        ...
        out_features: int,
        bias: bool = True,
        skip_value: float | list[float] = -100.0,
        skip_number: Optional[float] | list[float] = None,
    ):
        ...
        if skip_number is not None:
            ...
            self.skip_number = self.skip_value
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        if isinstance(self.skip_value, list):
            for i, skip_value in enumerate(self.skip_value):
                skip_mask = (src == skip_value).all(dim=-1)
                if skip_mask.any():
                    if isinstance(self.skip_number, list):
                        out[skip_mask] = self.skip_number[i]
                    else:
                        out[skip_mask] = self.skip_value[i]
        else:
            skip_mask = (src == self.skip_value).all(dim=-1)
            if skip_mask.any():
                assert isinstance(self.skip_number, float), (
                    f"skip_number must be a float, is {type(self.skip_number)}"
                )
                out[skip_mask] = self.skip_number
...
class MultiheadAttentionBlock(nn.TransformerEncoderLayer):
    ...
    def init_weights(self):
        ...
        if self.weight_range[0] == self.weight_range[1] == 0:
            ...
            nn.init.uniform_(
                self.linear1.weight, self.weight_range[0], self.weight_range[1]
            )
            nn.init.uniform_(
            ...
            nn.init.uniform_(
                self.linear2.weight, self.weight_range[0], self.weight_range[1]
            )
...
class InducedSelfAttentionBlock(nn.Module):
    ...
    def induced_attention(
        ...
        *batch_shape, _, d_model = src.shape
        ind_vectors = self.ind_vectors.expand(
    ...
    def forward(self, src: Tensor, train_size: Optional[int] = None) -> Tensor:
        ...
        if skip_mask.any():
            if skip_mask.all():
                ...
                out[~skip_mask] = self.induced_attention(
                    src[~skip_mask], train_size
                )
