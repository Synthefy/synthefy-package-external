...
class MemoryEstimator:
    ...
    def estimate_peak_mem(
        ...
        if include_inputs:
            ...
            mem_inputs = (
                n_elements * bytes_per_element / (1024**2)
            )  # Convert to MB
        ...
        def objective_function(bs: float) -> float:
            return (
                MemoryEstimator.estimate_peak_mem(
                    int(bs), seq_len, enc_name, include_inputs, in_dim
                )
                - target_memory
            )
...
class InferenceManager:
    ...
    def __init__(self, enc_name: str, out_dim: int, out_no_seq: bool = False, device: str = "cuda"):
        self.enc_name = enc_name
        self.out_dim = out_dim
        self.out_no_seq = out_no_seq
        self.device = device
        self._is_configured = (
            False  # Track if configure_inference has been called
        )
    ...
    def configure(
        ...
        if device is None:
            # TODO: fix, get this from config somehow
    ...
    def __call__(
        ...
        *batch_dims, seq_len, in_dim = first_value.shape
        ...
        if self.offload == "auto":
            ...
            output_mb = (
                bytes_per_element * math.prod(output_shape) / (1024 * 1024)
            )
        ...
        outputs = torch.empty(
            output_shape, dtype=input_dtype, device=output_device
        )
        ...
        while True:
            try:
                ...
                split_sizes = self.compute_split_sizes(
                    tuple(batch_dims), batch_size
                )
                # Create batches based on the calculated structure
                n_batches = self.compute_n_batches(
                    tuple(batch_dims), split_sizes
                )
                batch_iterator = self.create_multidim_batches(
                    inputs, tuple(batch_dims), split_sizes
                )
    ...
    def compute_split_sizes(
        batch_dims: Tuple[int, ...], batch_size: int
    ) -> List[int]:
    ...
    def compute_n_batches(
        batch_dims: Tuple[int, ...], split_sizes: List[int]
    ) -> int:
