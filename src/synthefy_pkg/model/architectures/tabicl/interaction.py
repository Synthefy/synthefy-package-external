...
class RowInteraction(nn.Module):
    ...
    def __init__(
        ...
        aggregate_operation: str = "select_last",
        ...
        # how the values are aggregated at the end, which changes the shape of the output
    ...
    def _aggregate_embeddings(
        ...
        outputs = self.tf_row(
            embeddings, key_padding_mask=key_mask
        )  # (B, T, H+C, E)
        if self.aggregate_operation == "select_last":
            cls_outputs = outputs[
                ..., : self.num_cls, :
            ].clone()  # (B, T, C, E)
        elif self.aggregate_operation == "all":
            cls_outputs = outputs.clone()  # (B, T, H, E)
        ...
        if self.aggregate_operation == "select_last":
            ...
            return cls_outputs  # (B, T, C, E)
        else:
            raise ValueError(
                f"Invalid aggregate operation: {self.aggregate_operation}"
            )

    def _train_forward(
        self, embeddings: Tensor, d: Optional[Tensor] = None
    ) -> Tensor:
        ...
        if d is None:
            ...
            d = d + self.num_cls
            indices = (
                torch.arange(HC, device=device).view(1, 1, HC).expand(B, T, HC)
            )
            key_mask = indices >= d.view(B, 1, 1).to(device)  # (B, T, HC)
    ...
    def _inference_forward(
        self, embeddings: Tensor, mgr_config: Optional[MgrConfig] = None
    ) -> Tensor:
