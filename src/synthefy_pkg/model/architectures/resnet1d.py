...
from synthefy_pkg.model.architectures.basic_conv1d import create_head1d
...
class BasicBlock1d(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, kernel_size=[3, 3], downsample=None
    ):
        super().__init__()
        ...
        self.conv1 = conv(
            inplanes, planes, stride=stride, kernel_size=kernel_size[0]
        ).type(torch.float32)
        self.bn1 = nn.BatchNorm1d(planes).type(torch.float32)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv(planes, planes, kernel_size=kernel_size[1]).type(
            torch.float32
        )
        self.bn2 = nn.BatchNorm1d(planes).type(torch.float32)
...
class Bottleneck1d(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, stride=1, kernel_size=3, downsample=None
    ):
        super().__init__()
...
class ResNet1d(nn.Sequential):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, kernel_size=3):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, stride, kernel_size, downsample)
        )
...
def bn_relu_conv1d(ni, nf, ks, stride, init_zero=False):
    ...
    return nn.Sequential(
        bn_initzero, nn.ReLU(inplace=True), conv1d(ni, nf, ks, stride)
    )
...
def _make_group(N, ni, nf, block, stride, drop_p, ks=3):
    return [
        block(ni if i == 0 else nf, nf, stride if i == 0 else 1, drop_p, ks=ks)
        for i in range(N)
    ]
...
class WideResNet1d(nn.Sequential):
    def __init__(
        ...
        for i in range(num_groups):
            n_channels.append(
                start_nf if fix_feature_dim else start_nf * (2**i) * k
            )

        layers: List[Union[nn.Module, nn.Sequential]] = [
            conv1d(input_channels, n_channels[0], 3, 1)
        ]  # conv1 stem
        for i in range(num_groups):
            layers += _make_group(
                N,
