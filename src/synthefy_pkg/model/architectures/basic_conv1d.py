...
def _conv1d(
    ...
    conv = conv.type(torch.float32)
    lst.append(conv)

    if bn:
        bn_layer = nn.BatchNorm1d(out_planes)
        bn_layer = bn_layer.type(torch.float32)
        lst.append(bn_layer)
...
def _fc(in_planes, out_planes, act="relu", bn=True):
    lst: list[nn.Module] = [nn.Linear(in_planes, out_planes, bias=not (bn))]
    if bn:
...
def weight_init(m):
    ...
    if isinstance(m, SqueezeExcite1d):
        stdv1 = math.sqrt(2.0 / m.w1.shape[0])
        nn.init.normal_(m.w1, 0.0, stdv1)
        stdv2 = math.sqrt(1.0 / m.w2.shape[1])
...
def listify(p: OptListOrItem = None, q: OptListOrItem = None):
    ...
    # Rank 0 tensors in PyTorch are Iterable but don't have a length.
        ...
        try:
            ...
            p = [p]
    # Only call len() on q if it's a collection that supports it
    if isinstance(q, int):
        n = q
    elif q is None:
        n = len(p)
    elif isinstance(q, (list, tuple, Collection)) and not isinstance(
        q, (str, float, int)
    ):
        n = len(q)
    else:
...
def create_head1d(
    ...
    first_dim = 2 * nf if concat_pooling else nf
    if lin_ftrs is None:
        lin_ftrs = [nc]
    else:
        lin_ftrs = list(lin_ftrs) + [nc]
    lin_ftrs = [first_dim] + lin_ftrs

    if isinstance(ps, (int, float)):
        ps = [ps]

    if len(ps) == 1:
        ps = [ps[0] / 2] * (len(lin_ftrs) - 2) + list(ps)
    ...
    for ni, no, p, actn in zip(lin_ftrs[:-1], lin_ftrs[1:], ps, actns):
        layers.extend(bn_drop_lin(ni, no, bn, p, actn))
        ...
        for i in range(len(filters)):
            ...
            layers_tmp.append(
                _conv1d(
                    ...
                    stride=(
                        1 if (split_first_layer is True and i == 0) else stride
                    ),
                    dilation=dilation,
                    act=(
                        "none"
                    ...
                    bn=False
                    if (headless is True and i == len(filters) - 1)
                    else bn,
            ...
            if squeeze_excite_reduction > 0:
                layers_tmp.append(
                    SqueezeExcite1d(filters[i], squeeze_excite_reduction)
                )
