...
class XSampler:
    ...
    def sample(self, return_numpy=False, return_signal_types=False):
        ...
        samplers = {
            ...
            "multinomial": self.sample_multinomial,
            "zipf": self.sample_zipf,
        }
    ...
    def sample_normal_all(self):
        ...
        return X, ["normal"] * self.num_features

    def sample_uniform(self):
        """Generate uniformly distributed features."""
        return (
            torch.rand((self.seq_len, self.num_features), device=self.device)
            - 0.5
        ) * 2, ["uniform"] * self.num_features
    ...
    def sample_normal(self, n=None):
        ...
        if self.pre_stats:
            return torch.normal(
                self.means[:, n], self.stds[:, n].abs()
            ).float(), ["normal"]
        else:
            return torch.normal(
                0.0, 1.0, (self.seq_len,), device=self.device
            ).float(), ["normal"]
    ...
    def sample_multinomial(self):
        ...
        x = x.float()
        return (x - x.mean()) / x.std(), ["multinomial"]
    ...
    def sample_mixed(self):
        ...
        for n in range(self.num_features):
            if random.random() > normal_p:
                x, signal_type = self.sample_normal(n)
        ...
        return torch.stack(X, -1), signal_types
