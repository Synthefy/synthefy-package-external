...
class AustinWaterDataloader(BaseEvalDataloader):
    """
    Dataloader for Austin water consumption time series data.
    """
    ...
    def _collect_files(self):
        path = "s3://synthefy-fm-eval-datasets/austin_water/"
    ...
    def __iter__(self) -> Iterator[EvalBatchFormat]:
        for file_path in self.csv_files:
            ...
            target_cols = ["total_gallons"]
            metadata_cols = [
                col
                for col in df.columns
                if col not in ["timestamp", "total_gallons"]
            ]

            # monthly data from 2012-2024
            eval_batch = EvalBatchFormat.from_dfs(
                dfs=[df],
                timestamp_col="timestamp",
                num_target_rows=24,  # 2 year forecast
