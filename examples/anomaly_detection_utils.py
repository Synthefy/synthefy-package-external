...
def load_anomaly_results(results_filename):
    """Load saved anomaly detection results from JSON files"""
    with open(f"{results_filename}.json", "r") as f:
        results = json.load(f)
    with open(f"{results_filename}_concurrent.json", "r") as f:
        concurrent_results = json.load(f)

    # Convert loaded results into AnomalyMetadata objects
    converted_results = {}
    for kpi, kpi_results in results.items():
        converted_results[kpi] = {}
        for anomaly_type, type_results in kpi_results.items():
            converted_results[kpi][anomaly_type] = {}
            for group_key, anomalies in type_results.items():
                converted_results[kpi][anomaly_type][group_key] = [
                    AnomalyMetadata(
        ...
        try:
            ...
            for kpi, grouped_anomalies in kpi_group_anomalies.items():
                for group_key, group_anomalies in grouped_anomalies.items():
                    ...
                    if current_ax == 0:
                        legend_elements = [
                            Line2D([0], [0], color="blue", label="Time Series (GT)"),
                            Line2D(
                                [0],
                                [0],
                                color="red",
                                linestyle="--",
                                label="Anomaly Time (Inferred)",
                            ),
                            # Line2D(
                            #     [0],
                            #     [0],
                            #     marker="o",
                            #     color="w",
                            #     label="Value at Anomaly Point (GT)",
                            ...
                            #     marker="o",
                            #     color="w",
                            #     label="Value at Anomaly Point (Inferred)",
                            #     markerfacecolor="red",
                            #     markersize=4,
                            # ),
                        ]
...
def save_anomaly_results(
    ...
    output_file_concurrent = f"{output_path}/{filename}_concurrent.json"
...
def plot_concurrent_group_anomalies(
    df: pd.DataFrame,
    concurrent_results: Dict[str, Dict[str, Any]],
    group_name: str,
    anomaly_type: str,  # 'peak', 'scattered', 'out_of_pattern', or 'mixed'
    timestamps_col: List[str],
    save_path: Optional[str] = None,
    figsize: Tuple[int, int] = (
        15,
        0,
    ),  # Height will be adjusted based on number of KPIs
    window_hours: int = 12,  # Hours before and after the concurrent anomalies to show
) -> None:
    """
    Plot concurrent anomalies for a specific group across all KPIs.
    ...
    Args:
        ...
        save_path: Optional path to save the plot
        figsize: Base figure size (width, height). Height will be adjusted based on number of KPIs
        window_hours: Hours before and after the concurrent anomalies to show
    """
    # Filter anomalies for the specified group and type
    relevant_timestamps = []
    kpis_with_anomalies = set()
    anomaly_types = (
        [anomaly_type]
        if anomaly_type != "mixed"
        else ["peak", "scattered", "out_of_pattern"]
    ...
    # Collect all relevant timestamps and KPIs
    for timestamp_key, cluster in concurrent_results.items():
        for anomaly in cluster["anomalies"]:
            if anomaly["group_key"] == group_name and (
                anomaly_type == "mixed" or anomaly["type"] == anomaly_type
            ):
                relevant_timestamps.append(pd.Timestamp(cluster["timestamp"]))
                kpis_with_anomalies.add(anomaly["kpi"])

    if not relevant_timestamps:
        logger.warning(
            f"No concurrent anomalies found for group {group_name} with type {anomaly_type}"
        )
        return

    kpis_with_anomalies = sorted(list(kpis_with_anomalies))
    n_kpis = len(kpis_with_anomalies)
    ...
    # Add anomaly type specific legend elements for mixed type
    if anomaly_type == "mixed":
        legend_elements.extend(
            [
                Line2D(
    ...
    # Get the full time range to plot
    min_timestamp = min(relevant_timestamps) - pd.Timedelta(hours=window_hours)
    max_timestamp = max(relevant_timestamps) + pd.Timedelta(hours=window_hours)

    # Create group filter
    group_filter = pd.Series(True, index=df.index)
    for timestamp_key, cluster in concurrent_results.items():
        for anomaly in cluster["anomalies"]:
            if anomaly["group_key"] == group_name:
                for col, val in anomaly["group_metadata"].items():
                    group_filter &= df[col] == val
                break
        if group_filter.any():
    ...
    # Get data for the group within the time window
    kpi_data = df[group_filter].copy()
    kpi_data[timestamps_col[0]] = pd.to_datetime(kpi_data[timestamps_col[0]])
    kpi_data = kpi_data[
        (kpi_data[timestamps_col[0]] >= min_timestamp)
        & (kpi_data[timestamps_col[0]] <= max_timestamp)
    ]

    # Plot each KPI
    for idx, kpi in enumerate(kpis_with_anomalies):
        ax = axes[idx]
        ...
        for timestamp_key, cluster in concurrent_results.items():
            cluster_time = pd.Timestamp(cluster["timestamp"])
            if min_timestamp <= cluster_time <= max_timestamp:
                for anomaly in cluster["anomalies"]:
                    if (
                        anomaly["group_key"] == group_name
                        and anomaly["kpi"] == kpi
                        and (anomaly_type == "mixed" or anomaly["type"] == anomaly_type)
                    ):
                        ...
                        # Plot vertical line and anomaly point
                        ax.axvline(
                            x=cluster_time, color=color, linestyle="--", alpha=0.3
                        )
                        # ax.scatter(
                        #     cluster_time,
                        #     anomaly["original_value"],
    ...
    # Add overall title
    plt.suptitle(
        f"Concurrent Anomalies for Group: {group_name}\nType: {anomaly_type.title()}",
        fontsize=16,
        y=1.02,
    )
    ...
    if save_path:
        os.makedirs(save_path, exist_ok=True)
        timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"concurrent_group_{group_name}_{anomaly_type}_{timestamp_str}.png"
        plt.savefig(os.path.join(save_path, filename), bbox_inches="tight", dpi=300)
        plt.close()
        logger.info(
            f"Saved concurrent group anomaly plot to: {os.path.join(save_path, filename)}"
        )
    else:
        plt.show()
